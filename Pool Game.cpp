...
void RenderScene(void) {
	...
	{
		glPushMatrix();
		glTranslatef(gTable.pockets[i].position(0), (POCKET_RADIUS / 2.0), gTable.pockets[i].position(1));
		#if DRAW_SOLID
		glutSolidSphere(gTable.pockets[i].radius, 32, 32);
		#else
		glutWireSphere(gTable.pockets[i].radius, 12, 12);
		#endif
		glPopMatrix();
		glColor3f(0.0, 0.0, 1.0);
