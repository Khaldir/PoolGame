...
/*-----------------------------------------------------------
  ...
  -----------------------------------------------------------*/
pocket::pocket(float x, float z, int i)
{
	position = vec2(x,z);
	index = i;
...
bool ball::HasHitPocket(const pocket &p) const
{
	...
	//if moving away, cannot have hit
...
{
	pockets[0] = pocket(TABLE_X, TABLE_Z, 0);
	pockets[1] = pocket(TABLE_X, -TABLE_Z, 1);
	pockets[2] = pocket(-TABLE_X, -TABLE_Z, 2);
	pockets[3] = pocket(-TABLE_X, TABLE_Z, 3);
}
